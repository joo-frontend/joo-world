    ...
    Z.dateRangePickerLanguages = {
        ...
        return Z(this).off(".datepicker").on("click.datepicker", function (e) {
            ...
            Z(this).data("date-picker-opened", !0), (m = function () {
                ...
                if (p.customArrowNextSymbol && (r = p.customArrowNextSymbol), t += '<div class="month-wrapper">   <table class="month1" cellspacing="0" border="0" cellpadding="0">       <thead>           <tr class="caption">               <th>                   <span class="prev">' + n + '                   </span>               </th>               <th colspan="' + a + '" class="month-name">               </th>               <th>' + (p.singleDate || !p.stickyMonths ? '<span class="next">' + r + "</span>" : "") + '               </th>           </tr>           <tr class="week-name">' + E() + "       </thead>       <tbody></tbody>   </table>", p.singleMonth || (t += '<div class="gap">' + function () {
                    ...
                    if (p.customShortcuts) for (var i = 0; i < p.customShortcuts.length; i++) {
                        var d = p.customShortcuts[i];
                        t += '&nbsp;<span class="custom-shortcut"><a href="javascript:;" shortcut="custom">' + d.name + "</a></span>"
                    }
                    t += "</div>"
                }
                if (p.showCustomValues && (t += '<div class="customValues"><b>' + (p.customValueLabel || K("custom-values")) + "</b>", p.customValues)) for (var i = 0; i < p.customValues.length; i++) {
                    var l = p.customValues[i];
                    t += '&nbsp;<span class="custom-value"><a href="javascript:;" custom="' + l.value + '">' + l.name + "</a></span>"
                }
                return Z(t += "</div></div>")
            }().hide()).append('<div class="date-range-length-tip"></div>'), Z(p.container).append(m), p.inline ? m.addClass("inline-wrapper") : i();
            ...
            setTimeout(function () {
                ...
                p.start = !1, p.end = !1, m.find(".day.checked").removeClass("checked"), p.setValue.call(o, e), M(), S(!0), j(), p.autoClose && N()
                ...
                if (-1 != t.indexOf("day")) {
                    ...
                    var o = Z(this).html();
                    if (p.customShortcuts && 0 < p.customShortcuts.length) for (var i = 0; i < p.customShortcuts.length; i++) {
                        var d = p.customShortcuts[i];
                        if (d.name == o) {
                            var l = [];
            ...
            setStart: function (e) {
                return "string" == typeof e && (e = Q(e, p.format).toDate()), p.end = !1, T(e), this
            }, setEnd: function (e, t) {
                var a = new Date;
                return a.setTime(p.start), "string" == typeof e && (e = Q(e, p.format).toDate()), O(a, e, t), this
            }, setDateRange: function (e, t, a) {
                "string" == typeof e && "string" == typeof t && (e = Q(e, p.format).toDate(), t = Q(t, p.format).toDate()), O(e, t, a)
            }, clear: function () {
                p.start = !1, p.end = !1, m.find(".day.checked").removeClass("checked"), m.find(".day.last-date-selected").removeClass("last-date-selected"), m.find(".day.first-date-selected").removeClass("first-date-selected"), p.setValue.call(o, ""), M(), S(), j()
            }, close: N, open: t, redraw: B, getDatePicker: function () {
                return m
            }, resetMonthsView: G, destroy: function () {
                Z(n).off(".datepicker"), Z(n).data("dateRangePicker", ""), Z(n).data("date-picker-opened", null), m.remove(), Z(window).off("resize.datepicker", i), Z(document).off("click.datepicker", U)
            }
        }), Z(window).on("resize.datepicker", i), this;
        ...
        function t(e) {
            B(), a(), p.customOpenAnimation ? p.customOpenAnimation.call(m.get(0), function () {
                ...
                Z(n).trigger("datepicker-opened", {relatedTarget: m})
            }), Z(n).trigger("datepicker-open", {relatedTarget: m}), I(), l(), i()
        }

        function a() {
            var e = p.getValue.call(o), t = e ? e.split(p.separator) : "";
            if (t && (1 == t.length && p.singleDate || 2 <= t.length)) {
                var a = p.format;
                a.match(/Do/) && (a = a.replace(/Do/, "D"), t[0] = t[0].replace(/(\d+)(th|nd|st)/, "$1"), 2 <= t.length && (t[1] = t[1].replace(/(\d+)(th|nd|st)/, "$1"))), s = !1, 2 <= t.length ? O(d(t[0], a, Q.locale(p.language)), d(t[1], a, Q.locale(p.language))) : 1 == t.length && p.singleDate && T(d(t[0], a, Q.locale(p.language))), s = !0
            }
        }
        ...
        function u(e, t) {
            m.find("." + e + " input[type=range].hour-range").val(Q(t).hours()), m.find("." + e + " input[type=range].minute-range").val(Q(t).minutes()), f(e, Q(t).format("HH"), Q(t).format("mm"))
        }
        ...
        function g(e) {
            if (!e.hasClass("invalid")) {
                var t = e.attr("time");
                if (e.addClass("checked"), p.singleDate ? (p.start = t, p.end = !1) : "week" === p.batchMode ? "monday" === p.startOfWeek ? (p.start = Q(parseInt(t)).startOf("isoweek").valueOf(), p.end = Q(parseInt(t)).endOf("isoweek").valueOf()) : (p.end = Q(parseInt(t)).endOf("week").valueOf(), p.start = Q(parseInt(t)).startOf("week").valueOf()) : "workweek" === p.batchMode ? (p.start = Q(parseInt(t)).day(1).valueOf(), p.end = Q(parseInt(t)).day(5).valueOf()) : "weekend" === p.batchMode ? (p.start = Q(parseInt(t)).day(6).valueOf(), p.end = Q(parseInt(t)).day(7).valueOf()) : "month" === p.batchMode ? (p.start = Q(parseInt(t)).startOf("month").valueOf(), p.end = Q(parseInt(t)).endOf("month").valueOf()) : p.start && p.end || !p.start && !p.end ? (p.start = c(t), p.end = !1) : p.start && (p.end = v(t), p.time.enabled && h("end", p.end)), p.time.enabled && (p.start && h("start", p.start), p.end && h("end", p.end)), !p.singleDate && p.start && p.end && p.start > p.end) {
                    var a = p.end;
                    p.end = v(p.start), p.start = c(a), p.time.enabled && p.swapTime && (u("time1", p.start), u("time2", p.end))
                }
        ...
        function D(e) {
            var t = e.val(), a = e.attr("name"), n = e.parents("table").hasClass("month1") ? "month1" : "month2",
                ...
                o = !!p.endDate && Q(p.endDate), i = Q(p[n])[a](t);
            s && i.isSameOrBefore(s) && (i = s.add("month2" === n ? 1 : 0, "month")), o && i.isSameOrAfter(o) && (i = o.add(p.singleMonth || "month1" !== n ? 0 : -1, "month")), z(i, n), "month1" === n ? (p.stickyMonths || Q(i).isSameOrAfter(p[r], "month")) && z(Q(i).add(1, "month"), r) : (p.stickyMonths || Q(i).isSameOrBefore(p[r], "month")) && z(Q(i).add(-1, "month"), r), I()
        }
        ...
        function S(e, t) {
            var a;
            m.find(".start-day").html("..."), m.find(".end-day").html("..."), m.find(".selected-days").hide(), p.start && m.find(".start-day").html(W(new Date(parseInt(p.start)))), p.end && m.find(".end-day").html(W(new Date(parseInt(p.end)))), p.start && p.singleDate ? (m.find(".apply-btn").removeClass("disabled"), a = W(new Date(p.start)), p.setValue.call(o, a, W(new Date(p.start)), W(new Date(p.end))), s && !t && Z(n).trigger("datepicker-change", {
                value: a,
                date1: new Date(p.start)
            })) : p.start && p.end ? (m.find(".selected-days").show().find(".selected-days-num").html(C(p.end, p.start)), m.find(".apply-btn").removeClass("disabled"), a = W(new Date(p.start)) + p.separator + W(new Date(p.end)), p.setValue.call(o, a, W(new Date(p.start)), W(new Date(p.end))), s && !t && Z(n).trigger("datepicker-change", {
                value: a,
                date1: new Date(p.start),
        ...
        function z(e, t) {
            var a = function (e, t) {
                var a, n = !!p.startDate && Q(p.startDate).add(p.singleMonth || "month2" !== t ? 0 : 1, "month"),
                    r = !!p.endDate && Q(p.endDate).add(p.singleMonth || "month1" !== t ? 0 : -1, "month");
                if (e = Q(e), !p.monthSelect || n && r && n.isSame(r, "month")) return '<div class="month-element">' + A(e.get("month")) + "</div>";
                if ((a = [n && e.isSame(n, "year") ? n.get("month") : 0, e ? e.get("month") : 11])[0] === a[1]) return '<div class="month-element">' + A(e.get("month")) + "</div>";
                return V("month", P(a, e.get("month"), function (e) {
                    return A(e)
                }))
                ...
                var a, n = !!p.startDate && Q(p.startDate).add(p.singleMonth || "month2" !== t ? 0 : 1, "month"),
                    r = !!p.endDate && Q(p.endDate).add(p.singleMonth || "month1" !== t ? 0 : -1, "month"),
                    s = e.get("year"), o = p.yearSelect && "function" == typeof p.yearSelect;
                if (!p.yearSelect || n && r && n.isSame(Q(r), "year")) return '<div class="month-element">' + s + "</div>";
                return a = o ? p.yearSelect(s) : p.yearSelect.slice(), V("year", P(a = [n ? Math.max(a[0], n.get("year")) : Math.min(a[0], s), r ? Math.min(a[1], r.get("year")) : Math.max(a[1], s)], s))
            }(e, t);
            m.find("." + t + " .month-name").html(a + " " + n), m.find("." + t + " tbody").html(function (e) {
                ...
                if (0 < s) for (var o = s; 0 < o; o--) {
                    ...
                    n = k(i.getTime()), p.startDate && F(i, p.startDate) < 0 && (n = !1), p.endDate && 0 < F(i, p.endDate) && (n = !1), t.push({
                        date: i,
                        type: "lastMonth",
                        day: i.getDate(),
                        time: i.getTime(),
                ...
                for (var d = e.getMonth(), o = 0; o < 40; o++) a = Q(e).add(o, "days").toDate(), n = k(a.getTime()), p.startDate && F(a, p.startDate) < 0 && (n = !1), p.endDate && 0 < F(a, p.endDate) && (n = !1), t.push({
                    date: a,
                ...
                for (var l = [], u = 0; u < 6 && "nextMonth" != t[7 * u].type; u++) {
                    ...
                    for (var i = 0; i < 7; i++) {
                        ...
                        if (a.extraClass = "", a.tooltip = "", a.valid && p.beforeShowDay && "function" == typeof p.beforeShowDay) {
                            ...
                            a.valid = f[0], a.extraClass = f[1] || "", a.tooltip = f[2] || "", "" !== a.tooltip && (a.extraClass += " has-tooltip ")
                        }
                        var c = {
                            time: a.time,
                        ...
                        0 === i && p.showWeekNumbers && l.push('<td><div class="week-number" data-start-time="' + a.time + '">' + p.getWeekNumber(a.date) + "</div></td>"), l.push("<td " + $({}, p.dayTdAttrs, a) + "><div " + $(c, p.dayDivAttrs, a) + ">" + J(a.time, a.day) + "</div></td>")
                    }
                    l.push("</tr>")
                }
                return l.join("")
