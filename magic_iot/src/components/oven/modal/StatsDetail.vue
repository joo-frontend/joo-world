  ...
  <div class="popup_detail">
    ...
    <div class="popup_detail_contents detail_cook_statistic">
        <div class="area_select">
            <select class="select select_block select_rounded" v-model="mode" v-on:change="searchCookList">
                <option v-for="item in this.modeList.list" v-bind:value="item.cookCateSeq" >{{item.cookCateName}}</option>
            </select>
        </div>
        <p class="period">• 기간 : {{this.cookInfo.startDate}}~{{this.cookInfo.endDate}}</p>
        <ul>
            <li v-for="item in this.cookInfo.statList" :key="item.seq" @click="showCookDetail(item)"><p>{{item.name}}</p> <button>{{item.cnt}}회 <span class="sprite sprite_steam_oven sprite_cook_detail_more"></span></button></li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    searchCookList: function () {
      this.$http.post('/api/stats/oven/cook', {
